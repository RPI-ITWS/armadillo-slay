...
router.get('/:state/:county', async (req, res) => {
    ...
    const query = validate(county, state) ? { county: county, state: state } : throw new Error("Invalid query");
...
router.delete('/:state/:county', async (req, res) => {
    ...
    result ? res.json({success: "Data deleted"}): res.json({error: "Data not deleted"});
