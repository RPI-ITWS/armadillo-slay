    ...
    return (
      <Container fluid="lg">
        <div className="text-center">
          <h2 className="fs-3">
            No Results Found. Please Try Again or add the county to the
            database.
          </h2>
        </div>
      </Container>
    ...
    return (
      <Container fluid="lg">
        <p className="text-center fs-4">
          Showing Data for {props.tableData[0].county},{" "}
          {props.tableData[0].state}
        </p>
        <p className="text-center fs-4">
          Average Household Income: ${props.tableData[0].householdIncome}
        </p>
        <Table striped bordered>
          ...
          <tbody>
            {months.map((month, monthIndex) => (
              <tr key={month}>
                <td>{month}</td>
                {props.tableData[0].monthlyData
                  .slice(0, -6)
                  .map((_: any, columnIndex: any) => (
                    <td key={`${month}-${columnIndex}`}>
                      {
                        props.tableData[0].monthlyData[columnIndex].monthlyData[
                          monthIndex
                        ].value
                      }
                    </td>
